...
const Middle = ({ loading = false,  }) => {
  const { sessionId } = useParams();
    const {  getSessionById, sessions = [], currentSession = []} = useSession();
    const summaries =currentSession;
    const {browserInfo: {browserName, userAgent} ={}, tabInfo:{favIconUrl,url, title} ={}} = sessions[0];
    ...
    const browserIcons = {
      ...
      default: <Globe className="w-5 h-5 text-gray-800 m-2" color="#000000" />, // Generic fallback icon
    };
      ...
      {loading
        ...
        : summaries?.map((item, idx) => (
              <div key={idx} className="flex flex-col justify-evenly">
                <div
              className="relative bg-gray-50 border-l-4 border-green-500 p-3 sm:p-4 rounded-lg shadow-sm mb-2"
            >
              {/* Section Title */}
            ...
            <div
              className="relative bg-green-50 border-r-4 border-green-500 p-3 sm:p-4 rounded-lg shadow-sm mb-2"
            >
              {/* Section Title */}
              <div className="flex items-center mb-2">
                <span className="bg-gray-200 text-gray-800 px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-md text-[10px] sm:text-xs font-medium mr-1 sm:mr-2">
